...
class Empresa
{
    ...
    public function login()
    {
        ...
        $bd = new Conexion();
        $sql = "SELECT * FROM empresa WHERE (NOMBRE = ? OR TELEFONO = ? OR SITIO_WEB = ? OR CONTACTO_CORREO = ? OR email = ?) AND PASSWORD = ?";

        // Utilizar una consulta preparada
        $stmt = $bd->prepare($sql);
        $stmt->bind_param("ssssss", $this->nombre, $this->sitioWeb, $this->contactoCorreo, $this->telefono, $this->email, $this->password);
